"""E2E tests for AI agents using real OpenAI provider.

Requires OPENFATTURE_AI_OPENAI_API_KEY to be set.
Run with: pytest -m "openai and e2e"
"""
...
@pytest.mark.openai
@pytest.mark.e2e
class TestOpenAIInvoiceAssistant:
    """Test InvoiceAssistant with real OpenAI provider."""
    ...
    async def test_response_latency(self, openai_provider):
    ...
    async def test_token_usage_tracking(self, openai_provider):
        ...
        cost = openai_provider.estimate_cost(response.usage)
        assert cost > 0 and cost < 1.0  # Should be less than $1 for simple request

    async def test_malformed_prompt_handling(self, openai_provider):
...
@pytest.mark.openai
@pytest.mark.e2e
class TestOpenAITaxAdvisor:
    """Test TaxAdvisor with real OpenAI provider."""

    async def test_standard_vat_rate(self, openai_provider):
        """Test standard VAT rate recommendation."""
        agent = TaxAdvisorAgent(provider=openai_provider)
        context = TaxContext(
    ...
    async def test_split_payment_public_administration(self, openai_provider):
    ...
    async def test_exempt_services(self, openai_provider):
    ...
    async def test_foreign_client_export(self, openai_provider):
    ...
    async def test_reduced_vat_rate(self, openai_provider):
    ...
    async def test_regime_forfettario(self, openai_provider):
...
class TestOpenAIProviderCapabilities:
    """Test OpenAI provider specific capabilities."""
    ...
    async def test_tool_calling_support(self, openai_provider):
        """Test that OpenAI provider supports tool calling."""
        assert openai_provider.supports_tools

    async def test_token_counting_accuracy(self, openai_provider):
        """Test token counting is reasonable."""
        text = "Questo è un testo di prova per contare i token."
        tokens = openai_provider.count_tokens(text)

        # Should be reasonable estimate (Italian text)
        assert 10 <= tokens <= 20

        # Empty text should be 0
        assert openai_provider.count_tokens("") == 0

    async def test_cost_estimation(self, openai_provider):
        ...
        low_cost = openai_provider.estimate_cost(low_usage)
        assert low_cost > 0

        # High usage
        high_usage = UsageMetrics(prompt_tokens=1000, completion_tokens=500, total_tokens=1500)
        high_cost = openai_provider.estimate_cost(high_usage)
        assert high_cost > low_cost  # Higher usage should cost more

    async def test_health_check_reliability(self, openai_provider):
    ...
    async def test_error_handling_rate_limits(self, openai_provider):
