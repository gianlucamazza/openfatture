...
class BaseTemplate(ABC):
    ...
    def draw_bollo_footer(self, canvas: Canvas, importo_bollo: Decimal, bollo_assolto_virtuale: bool, y_position: float) -> float:
        ...
        canvas.drawString(
            2.3 * cm,
            y,
            f"Tipo: Bollo assolto ai sensi del decreto MEF 17 GIUGNO 2014 (ART. 6)",
        )
        
        # Show amount
        canvas.setFont("Helvetica", 9)
        canvas.drawString(11 * cm - 2 * cm, y_position - 0.5 * cm, f"Importo: € {importo_bollo:.2f}")
