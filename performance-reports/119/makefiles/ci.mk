...
ci: ci-install ci-lint ci-type-check ci-test ci-coverage ci-security ## Run complete CI pipeline
...
ci-type-check: ## Run type checking for CI
	...
	@MYPYPATH=stubs $(UV) run python -m mypy $(PROJECT_ROOT)/
...
security-safety: ## Check for known vulnerabilities
	...
	@$(UV) run safety check --json || echo "$(YELLOW)Some vulnerabilities found (check output)$(NC)"
...
security-audit: ## Run comprehensive security audit
	...
	@$(UV) run pip-audit || echo "$(YELLOW)pip-audit not installed$(NC)"
...
publish: build-check ## Publish to PyPI
	@echo "$(YELLOW)This will publish to PyPI!$(NC)"
