...
def configure_logging(
    ...
    # Logs go to stderr so machine-readable CLI output (e.g. status --json)
    # stays pure on stdout.
    logging.basicConfig(
        format="%(message)s",
        stream=sys.stderr,
...
def configure_dynamic_logging(debug_config: DebugConfig) -> None:
    ...
    configure_logging(
        ...
        json_logs=False,  # CLI-friendly text logs; set True for structured JSON
