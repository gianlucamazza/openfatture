...
4. **Check configuration**
   ...
   - Review updated email templates (`openfatture/platform/email/templates`).
5. **Regenerate knowledge base (if AI enabled)**
   ```bash
   uv run openfatture ai rag index
   ```

> No breaking changes to database identifiers; `init_db()` creates the new payment allocation tables automatically.
...
- Payment gateway integrations are scheduled for Phase 6.
