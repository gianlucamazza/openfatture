"""Unified async/sync bridge for CLI and scripts.

Primary path: ``asyncio.run`` when no loop is running.

Nested path (notebooks / async test runners): if a loop is already running,
prefer optional ``nest_asyncio``; otherwise run the coroutine in a dedicated
thread with its own loop. Prefer native ``async``/``await`` when the caller
is already async.
"""
...
def run_async[T](
    ...
    *,
    ...
    try:
        ...
        # Nested event loop (notebooks / some test runners). Prefer nest_asyncio
        # when installed; otherwise isolate in a worker thread.
        logger.debug(
        ...
        try:
            ...
            logger.info(
                "nested_loop_using_thread_pool",
                message="nest_asyncio not installed; running coroutine in a worker thread",
            )
